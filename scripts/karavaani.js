...
async function getListingIds() {
  ...
  const mainBlocks = Array.from(listingList.children).filter(
    (child) => child.id && child.id.includes("mainBlock")
  );
  const newBadges = [];
  ...
  const fetchCaravanDataPromises = listingInfoResults.map(
    async (listingInfo) => {
  ...
  for (let i = 0; i < mainBlocks.length; i++) {
    ...
    if (!existingBadge) {
      const badge = createBadgeElement(
        fetchCaravanDataResults[i],
        listingInfoResults[i],
        mainBlocks[i]
      );
...
function createBadgeElement(info, listing, mainBlock) {
  ...
  badge.addEventListener("click", async () => {
    if (!badge.querySelector(".infoBox")) {
      let [yearMin, yearMax, kmMin, kmMax, req] = await getYearAndKm(listing);

      const infoBox = document.createElement("div");
      infoBox.classList.add("infoBox");
      infoBox.textContent =
        "Vehicle Type: " +
        listing.vehicleType?.fi +
        ", Make: " +
        listing.make?.name +
        ", Model: " +
        listing.model?.name +
      ...
      yearOkButton.addEventListener("click", async () => {
        ...
        if (yearMinValue > yearMaxValue) {
          ...
          const newYearRequirements = await updateRequirements(
            listing,
            newYearRange,
            undefined
          );
          ...
          chrome.storage.local.set(
            { requirements: newYearRequirements },
            () => {
              if (chrome.runtime.lastError) {
                console.error(
                  "ERROR: requirements was not set to local storage"
                );
                alert(
                  "Ei voitu asettaa uusia vuosi vaatimuksia paikalliseen tallennustilaan"
                );
              } else {
        ...
        [yearMin, yearMax, undefined, undefined, req] = await getYearAndKm(
          listing
        );
        yearMinInput.value = yearMin;
        yearMaxInput.value = yearMax;

        let elements = document.querySelectorAll(".valueYearPair");

        if (elements.length != 0) {
          yearViewAllButton.click();
        }
      });

      yearViewAllButton.addEventListener("click", async () => {
        [yearMin, yearMax, undefined, undefined, req] = await getYearAndKm(
          listing
        );
        yearMinInput.value = yearMin;
        ...
        if (req) {
          for (let i = 0; i < req.year_range.length; i++) {
            if (req.year_range) {
              ...
              valueRemovePair.classList.add("valueYearPair");
      ...
      kmOkButton.addEventListener("click", async () => {
        ...
        if (kmMinValue > kmMaxValue) {
          ...
          const newkmRequirements = await updateRequirements(
            listing,
            undefined,
            newkmRange
          );
        ...
        [undefined, undefined, kmMin, kmMax, req] = await getYearAndKm(listing);
        ...
        let elements = document.querySelectorAll(".valueKmPair");

        if (elements.length != 0) {
          kmViewAllButton.click();
        }
      });

      kmViewAllButton.addEventListener("click", async () => {
        [undefined, undefined, kmMin, kmMax, req] = await getYearAndKm(listing);
        kmMinInput.value = kmMin.toLocaleString("fi-FI");
        kmMinInput.dataset.value = kmMin;
        kmMaxInput.value = kmMax.toLocaleString("fi-FI");
        ...
        if (req) {
          for (let i = 0; i < req.km_range.length; i++) {
            if (req.km_range) {
              ...
              valueRemovePair.style.display = "flex";
              valueRemovePair.style.flexDirection = "row";
              valueRemovePair.classList.add("valueKmPair");
              kmRangeDiv.appendChild(valueRemovePair);

              const value = document.createElement("h3");
              value.setAttribute("id", "value " + i);
              value.textContent =
                kmRange[0].toLocaleString("fi-FI") +
                " - " +
                kmRange[1].toLocaleString("fi-FI");
              valueRemovePair.appendChild(value);
      ...
      kmMinInput.addEventListener("input", () => {
        ...
        kmMinInput.dataset.value = Number(numericValue);
      ...
      kmMaxInput.addEventListener("input", () => {
        ...
        kmMaxInput.dataset.value = Number(numericValue);
...
async function getYearAndKm(listing) {
  ...
  if (req) {
    for (let i = 0; i < req.year_range.length; i++) {
      if (
        listing.year >= req.year_range[i][0] &&
        listing.year <= req.year_range[i][1]
      ) {
  ...
  if (req) {
    for (let i = 0; i < req.km_range.length; i++) {
      if (
        listing.kilometers >= req.km_range[i][0] &&
        listing.kilometers <= req.km_range[i][1]
      ) {
