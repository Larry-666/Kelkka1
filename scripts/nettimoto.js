let refToken = "45eb3d28a13d0f2ac4a88d82c8db373f87f269c1"
...
async function getListingIds() {
  ...
  const fetchBikeDataPromises = listingInfoResults.map(async listingInfo => {
    ...
    if (yearRequirements !== undefined && Array.isArray(yearRequirements)) {
      yearReq = yearRequirements.find(req =>
        listingInfo.bikeType.id === req.bikeType &&
        listingInfo.make.id === req.make &&
        listingInfo.model.id === req.model &&
...
function createBadgeElement(info, listing) {
  ...
  badge.addEventListener("click", async () => {
    if (!badge.querySelector(".infoBox")) {
      ...
      infoBox.textContent = "Bike Type: " + listing.bikeType?.fi + ", Make: " + listing.make?.name + ", Model: " + listing.model?.name + ", Year: " + listing.year + ", EngineStroke: " + listing.fuelType?.fi + ", Kilometers: " + listing.kilometers + ", Engine Size:" + listing.engineSize?.name
      ...
      if (yearRequirements !== undefined && Array.isArray(yearRequirements)) {
        yearReq = yearRequirements.find(req =>
          listing.bikeType.id === req.bikeType &&
          listing.make.id === req.make &&
          listing.model.id === req.model &&
...
async function updateYearRequirements(listingInfo, year_range) {
  ...
  const existingYearRequirement = yearRequirements.find((yearRequirement) => {
    return (
      yearRequirement.bikeType === listingInfo.bikeType.id &&
      yearRequirement.make === listingInfo.make.id &&
      yearRequirement.model === listingInfo.model.id &&
  ...
  if (existingYearRequirement) {
    ...
    yearRequirements.push({
      bikeType: listingInfo.bikeType.id,
      make: listingInfo.make.id,
      model: listingInfo.model.id,
